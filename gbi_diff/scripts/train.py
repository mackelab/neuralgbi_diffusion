...
from gbi_diff.model.lit_module import DenoiserModel, Guidance, PotentialNetwork
from gbi_diff.model.utils import (
    compute_multiplybymean_params,
    compute_standardizing_net_params,
)
from gbi_diff.utils.configs.train_denoiser import Config as Config_Denoiser
...
from gbi_diff.utils.metrics import mmd_dist, mse_dist
...
def train_potential(config: Config_Potential, devices: int = 1, force: bool = False):
    ...
    kwargs = {
        ...
        "distance_stats": compute_multiplybymean_params(
            dist_func, train_set._x_target, train_set._x
        ),
    }
...
def train_denoiser(config: Config_Denoiser, devices: int = 1, force: bool = False):
    # TODO: fixup device config
    serial_config = config
    trainer, log_dir = _setup_trainer(config, devices)
    train_loader, train_set, val_loader, _ = _setup_datasets(config)
